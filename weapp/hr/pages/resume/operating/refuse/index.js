...
Page({
    ...
    formSubmit(e) {
        ...
        app.$http.post('candidate/v1/refuse', data).then(res => {
            if(res.success){
                wx.showToast({title: '拒绝成功'});
                // 埋点
                app.$handleBury({evt: 'hx3a'});
                setTimeout(() => {
                    wx.navigateTo({ url : `../eval/index?candidateId=${this.data.query.candidateId}` }); 
                }, 1000);
